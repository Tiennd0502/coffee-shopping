...
export const PageContent = () => {
  ...
  const { mutate: deleteCategoryMutate, isPending: isDeletePending } = useDeleteCategory();
  ...
  return (
    <div className="flex flex-col gap-6">
      <AlertDialog
        ...
        description={
          <p>
            This will remove <b>{pendingDelete?.name}</b> (<b>{pendingDelete?.slug}</b>). It may
            still appear in this list with the Removed badge.
          </p>
        }
        textAction="Delete"
        onClickAction={() => {
        ...
        {isLoading ? (
          ...
          <Table
            ...
            resolveRowClassName={(row) =>
              row.deletedAt ? 'border-b border-border bg-muted-foreground/20' : ''
            }
            renderRow={(row) => (
              <CategoryTableRow
                category={row}
