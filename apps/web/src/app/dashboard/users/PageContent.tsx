  ...
  const handleUserRoleUpdate = (user: User, nextRole: USER_ROLE) => {
    ...
    updateUserRoleMutate(
      { id: userId, role: nextRole },
      {
        ...
        onSettled: () => {
          setPendingRoleUserId((current) => (current === userId ? null : current));
        },
      },
    );
  };
  ...
  return (
    <div className="flex flex-col gap-6">
      <AlertDialog
        ...
        description={
          <p>
            Are you sure you want to delete <b>{pendingDeleteLabel}</b>? This action cannot be
            undone.
          </p>
        }
        textAction="Delete"
      ...
      <header className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex w-full justify-end sm:w-auto">
          <Button variant="default" size="default" disabled className="w-full gap-2 px-8 sm:w-auto">
        ...
        {isLoading ? (
          ...
          <Table
            ...
            resolveRowClassName={(user) =>
              user.deletedAt ? 'border-b border-border bg-muted-foreground/20' : ''
            }
            renderRow={(user) => (
              <UserTableRow
                user={
                ...
                isRoleDisabled={pendingRoleUserId === user.id || Boolean(user.deletedAt)}
