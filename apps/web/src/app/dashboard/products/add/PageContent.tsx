...
const PageContent = () => {
  ...
  const validateImageInputs = (): boolean => {
    const nextErrors: ImageValidationErrors = {
      ...
      gallery: galleryImages.length > 0 ? '' : ERROR_MESSAGES.PRODUCT_GALLERY_REQUIRED,
    };
  ...
  const uploadSelectedImages = async () => {
    const avatarUpload = avatarImage ? uploadImageToImgBB(avatarImage.file) : null;
            ...
            <article className="rounded-4xl border border-outline-variant/30 bg-surface-container-low p-6">
              ...
              <div className="mt-5 grid gap-4 md:grid-cols-2">
                ...
                <div className="md:col-span-2 grid gap-4 md:grid-cols-2">
                  ...
                  <Input
                    className="h-14"
                    label={isPercentDiscount ? 'Discount (%)' : 'Discount Amount'}
                    type="number"
                    min={0}
                    max={isPercentDiscount ? 100 : undefined}
                    step={isPercentDiscount ? 1 : 0.01}
                    endIcon={isPercentDiscount ? <span className="text-sm">%</span> : undefined}
                    disabled={isSubmitting}
