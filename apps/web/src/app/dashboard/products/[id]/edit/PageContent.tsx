...
const EditProductForm = ({ product, productId }: EditProductFormProps) => {
  ...
  const hasPrimaryImageChanged = avatarImage !== null || serverPrimaryUrl !== initialPrimaryUrl;
  const hasGalleryChanged =
    ...
    existingGalleryItems.some((item, index) => {
      ...
      return (
        item.id !== initialItem.id || item.url !== initialItem.url || item.name !== initialItem.name
      );
    });
