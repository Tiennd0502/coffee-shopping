...
jest.mock('@/components/Select', () => ({
  Select: ({
    ...
    const ariaLabel = placeholder === 'All statuses' ? 'Status filter' : 'Category filter';
    return (
      <label>
...
describe('Dashboard products page', () => {
  ...
  it('filters products by keyword via URL params and API params', async () => {
    ...
    fireEvent.change(screen.getByRole('searchbox', { name: 'Filter products by name' }), {
  ...
  it('opens remove dialog and calls delete mutate on confirm', async () => {
    ...
    await user.click(screen.getByRole('button', { name: /delete ethiopian yirgacheffe/i }));
    expect(await screen.findByText('Delete Product?')).toBeInTheDocument();

    await user.click(screen.getByRole('button', { name: /^delete$/i }));
    expect(mutateDeleteProduct).toHaveBeenCalledWith('product-1', expect.any(Object));
  });
  ...
  it('renders error state and retries', async () => {
    ...
    expect(screen.getByText(API_FALLBACK_ERRORS.PRODUCTS_LOAD)).toBeInTheDocument();
