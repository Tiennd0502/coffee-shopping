...
describe('getPaginationItems (Chakra-style siblingCount)', () => {
  ...
  it('early pages: fixed left block, ellipsis, last page', () => {
    ...
    expect(items.map((t) => (t.type === 'page' ? t.page : '…'))).toEqual([1, 2, 3, 4, 5, '…', 20]);
  });
  ...
  it('near end: first, ellipsis, trailing block', () => {
    ...
    expect(items.map((t) => (t.type === 'page' ? t.page : '…'))).toEqual([1, '…', 6, 7, 8, 9, 10]);
  });

  it('small total: all pages, no ellipsis', () => {
    const items = getPaginationItems(2, 7);
    expect(items).toEqual([1, 2, 3, 4, 5, 6, 7].map((page) => ({ type: 'page' as const, page })));
  });
});
...
describe('PaginationBar', () => {
  it('renders nothing when totalPages is 0', () => {
    const { container } = render(
      <PaginationBar currentPage={1} totalPages={0} onPageChange={jest.fn()} {...defaultSummary} />,
    );
    expect(container.querySelector('[data-slot="pagination"]')).not.toBeInTheDocument();
  });
  ...
  it('shows early block and last for many pages on page 1', () => {
    ...
    expect(document.querySelectorAll('[data-slot="pagination-ellipsis"]')).toHaveLength(1);
  });
