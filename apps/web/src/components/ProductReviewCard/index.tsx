...
export function ProductReviewCard({ review, className }: ProductReviewCardProps) {
  return (
    ...
    >
        ...
        <div className="min-w-0 flex-1 space-y-2">
          <div className="flex flex-wrap items-start justify-between gap-2">
            <div>
              ...
              <p className="text-xs text-on-surface-variant">{review.authorRole}</p>
            </div>
          </div>
      ...
      <time className="mt-4 text-xs text-on-surface-variant" dateTime={review.dateLabel}>
        {review.dateLabel}
      </time>
    </article>
  );
    ...
    <div className="flex gap-0.5" aria-label={`${rating} out of 5 stars`}>
      {Array.from({ length: 5 }, (_, i) => (
        <Star
          ...
          className={cn(
            'size-4 shrink-0',
            i < rating ? 'fill-primary-container text-primary-container' : 'text-outline-variant',
          )}
          aria-hidden
        />
      ))}
