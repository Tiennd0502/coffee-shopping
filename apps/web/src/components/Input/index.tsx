...
import { formatNumericWithThousands, normalizeNumericInput } from '@/utils/number';
import { cn } from '@/utils/styles';

export interface InputProps extends Omit<React.ComponentPropsWithoutRef<typeof Input>, 'type'> {
  id?: string;
...
const InputField = React.forwardRef<HTMLElement, InputProps>(function InputField(
  {
    id: idProp,
    label,
    text,
  ...
  const [uncontrolledFormattedNumber, setUncontrolledFormattedNumber] = React.useState(() => {
  ...
  const describedBy =
    [ariaDescribedByProp, hasHelperText ? descriptionId : undefined, hasError ? errorId : undefined]
      .filter(Boolean)
      .join(' ') || undefined;

  const isInvalidFromProp = ariaInvalidProp === true || ariaInvalidProp === 'true';
  const ariaInvalid = hasError || isInvalidFromProp ? true : ariaInvalidProp;

  const padEndClass = !hasTrailingSlot ? null : hasEndIcon && isPassword ? 'pr-20' : 'pr-12';
    ...
    >
      ...
      <div className="relative">
        ...
        <Input
          ...
          className={cn('peer', hasStartIcon && 'pl-12', padEndClass, className)}
        ...
        {hasTrailingSlot && (
          <div className="absolute top-1/2 right-3 z-10 flex -translate-y-1/2 items-center gap-1">
            {hasEndIcon && (
              <span className="flex text-on-surface-variant [&_svg]:size-5">{endIcon}</span>
            )}
            {isPassword && (
              <button
                type="button"
      ...
      {hasHelperText && (
        <p id={descriptionId} className="mt-2 text-xs text-on-surface-variant">
          {text}
        </p>
      )}
      {hasError && (
