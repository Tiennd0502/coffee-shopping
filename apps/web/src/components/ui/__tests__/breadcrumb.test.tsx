...
describe('Breadcrumb', () => {
  ...
  it('exposes ellipsis with screen-reader fallback', () => {
    ...
    expect(screen.getByText('More', { selector: '.sr-only' })).toBeInTheDocument();
    const ellipsisHost = document.querySelector('[data-slot="breadcrumb-ellipsis"]');
  ...
  it('wraps structure in breadcrumb data-slot on nav', () => {
    ...
    const item = within(nav).getByText('X').closest('[data-slot="breadcrumb-item"]');
