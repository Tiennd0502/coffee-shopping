...
import { ROAST_LEVEL_OPTIONS, ROAST_LEVEL_SPECTRUM_PERCENT } from '@/constants/roast';
...
export function ProductDetailSpecGrid({ product, className }: ProductDetailSpecGridProps) {
  const roastLabel =
    ROAST_LEVEL_OPTIONS.find((o) => o.value === product.roastLevel)?.label?.replace(' Roast', '') ??
    product.roastLevel;
  const spectrum = ROAST_LEVEL_SPECTRUM_PERCENT[product.roastLevel];
  const notes = product.tastingNotes?.trim() || product.description?.trim().slice(0, 80) || '—';
...
function SpecCard({
  ...
  return (
    <div className="rounded-xl border border-primary/10 bg-surface-container-low p-4 shadow-sm dark:border-border md:p-5">
      ...
      <p className={cn('mt-2 text-sm text-on-surface md:text-base', valueClassName)}>{value}</p>
      {children}
    </div>
  );
}
