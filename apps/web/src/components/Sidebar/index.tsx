    ...
    <>
                  ...
                  return (
                    <SidebarMenuItem key={item.href}>
                      <SidebarMenuButton
                        ...
                        tooltip={
                          isDisabled ? `${item.label} - ${DASHBOARD_DISABLED_HINT}` : item.label
                        }
      ...
      <SidebarFooter className="min-w-0 p-3 group-data-[collapsible=icon]:hidden">
        <div className="rounded-xl bg-sidebar-accent/80 p-4 text-sidebar-foreground">
          <p className="font-semibold text-sidebar-foreground">Inventory Alert</p>
          <p className="mt-1 text-sm text-muted-foreground">3 items are running low on stock.</p>
          <Button
            disabled
            variant="default"
