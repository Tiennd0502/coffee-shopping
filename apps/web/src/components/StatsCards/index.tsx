      ...
      <div className="space-y-3">
        <p
          className={cn(
            ...
            isAccent ? 'text-primary-foreground/70' : 'text-on-surface-variant/80',
          )}
        >
          {label}
        </p>
      ...
      {footnote && (
        <p
          className={cn(
            ...
            footnoteTone === 'muted' &&
              (isAccent ? 'text-primary-foreground/85' : 'text-on-surface-variant/80'),
          )}
        >
...
export function StatsCards({ items, className }: StatsCardsProps) {
  ...
  return (
    <section
      ...
      className={cn('grid grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-4', className)}
    >
      {items.map((item) => (
