...
describe('SearchInput', () => {
  ...
  it('allows placeholder override', () => {
    ...
    expect(screen.getByLabelText('Search')).toHaveAttribute('placeholder', 'Find coffee…');
  });
  ...
  it('merges containerClassName onto the wrapper', () => {
    ...
    expect(screen.getByLabelText('q').closest('[data-slot="search-input"]')).toHaveClass(
      'max-w-xs',
    );
  });
  ...
  it('applies pill and token classes on wrapper', () => {
    ...
    const wrap = screen.getByLabelText('p').closest('[data-slot="search-input"]');
