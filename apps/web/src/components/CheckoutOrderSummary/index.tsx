...
export const CheckoutOrderSummary = ({
  ...
  const hasItemErrors = items.some((item) => Boolean(itemQuantityErrors?.[item.id]));
  ...
  return (
    ...
    >
          ...
          return (
            <article
              ...
              className={cn(
                ...
                (hasLineError || lineOutOfStock) && 'border border-destructive/40 bg-destructive/5',
              )}
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="truncate text-sm text-on-surface-variant">{item.meta}</p>
                <p className="mt-1 font-semibold text-primary">{formatCurrency(item.unitPrice)}</p>
                {lineOutOfStock && (
                  <p className="mt-1 text-xs font-medium text-destructive">{OUT_OF_STOCK_LABEL}</p>
                )}
                {hasLineError && (
      ...
      <dl className="space-y-2 text-on-surface-variant">
        ...
        <div className="flex items-center justify-between gap-2">
          <dt>Shipping</dt>
          <dd>{totals.shipping === null ? 'Free' : formatCurrency(totals.shipping)}</dd>
        </div>
        <div className="flex items-center justify-between gap-2">
          <dt>Estimated Tax</dt>
      ...
      <p className="mt-6 text-center text-xs text-on-surface-variant">
        By placing your order, you agree to CoffeeHub&apos;s Terms of Service and Privacy Policy.
      </p>
    </aside>
  );
};
