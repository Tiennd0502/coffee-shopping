...
function readProductCategory(
  ...
  const category = categoryOptions.find((option) => String(option.value) === product.categoryId);
  if (!category) return '—';
  ...
  return (
    <>
        ...
        <div className="flex min-w-0 items-start gap-3">
          <div className="size-14 shrink-0 overflow-hidden rounded-sm bg-muted">
            <Avatar className="size-14 rounded-sm after:rounded-sm after:border-transparent">
              <AvatarImage src={imageUrl ?? ''} alt={productName} className="rounded-sm" />
          ...
          <div className="min-w-0 flex-1 space-y-0.5 text-left">
            <p title={productName} className="truncate font-semibold leading-tight text-foreground">
              {productName}
            </p>
            <p title={subtitle} className="line-clamp-2 text-sm text-muted-foreground">
              {subtitle}
            </p>
          </div>
        </div>
      ...
      <td className="px-6 py-4 align-middle">
        <p className="text-sm text-center font-semibold text-foreground">{stock}</p>
      </td>
      <td className="px-6 py-4 align-middle text-center">
        <Badge className={cn('h-7 px-3 text-[0.65rem] uppercase', getStatusStyles(status))}>
          {status}
        </Badge>
      </td>
      <td className="px-6 py-4 align-middle">
