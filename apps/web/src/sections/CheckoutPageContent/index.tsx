...
const CheckoutPageContent = () => {
  ...
  const [submitErrors, setSubmitErrors] = useState<ApiErrorResponse | null>(null);
  ...
  const isPlaceOrderDisabled = isLoading || isSubmitting || !hasItems || hasOutOfStockItems;
  ...
  useEffect(() => {
    setValues((previousValues) => ({
      ...
      district:
        previousValues.district.length > 0 ? previousValues.district : initialUserValues.district,
      ward: previousValues.ward.length > 0 ? previousValues.ward : initialUserValues.ward,
  ...
  const handleAddressSuggestionSelect = (suggestion: MapboxAddressSuggestion) => {
    const parsed = mapboxFeatureToCheckoutAddress(suggestion.feature);
    const props = suggestion.feature.properties ?? {};
    const streetNumber = typeof props.address === 'string' ? props.address.trim() : '';
    ...
    setValues((previousValues) => ({
      ...
      district: parsed.district.trim() ? parsed.district : previousValues.district,
      ward: parsed.ward.trim() ? parsed.ward : previousValues.ward,
      postalCode: parsed.postalCode.trim() ? parsed.postalCode : previousValues.postalCode,
    }));
    setFieldErrors((previousErrors) => ({
      ...previousErrors,
      addressLine: undefined,
        ...
        onError: (error) => {
          ...
          if (responseData && typeof responseData === 'object') {
            ...
            setSubmitErrors({
              ...
              message: typeof data.message === 'string' ? data.message : defaultMessage,
  ...
  const hasRootSubmissionError = Boolean(
    submitErrors && (submitErrors.errors?.length ?? 0) === 0 && Boolean(submitErrors.message),
  );

  const hasBlockingItemErrors = hasVisibleLineSubmitErrors || hasRootSubmissionError;
  ...
  if (isLoading) {
    return (
      <div className="mx-auto max-w-7xl px-4 py-8 md:px-6 md:py-12" data-testid="checkout-loading">
        <Loading />
      </div>
    );
  }
  ...
  if (isError) {
    return (
      ...
      >
        <h1 className="text-3xl font-semibold text-on-surface">Finalize Your Brew Order</h1>
  ...
  if (!hasItems) {
    return (
      ...
      >
        ...
        <Button className="mt-6 w-auto px-8" onClick={() => router.push(ROUTES.CART)}>
          Back to Cart
        </Button>
      </div>
    );
      ...
      <div className="mt-10 grid gap-8 lg:grid-cols-[minmax(0,1fr)_360px]">
        <form ref={formRef} className="space-y-8" onSubmit={handleSubmit} noValidate>
          <section aria-label="Shipping Information">
            <div className="flex items-center justify-start gap-2">
              ...
              <h2 className="text-2xl font-medium text-on-surface">Shipping Information</h2>
            </div>
            ...
            {(isAddressLookupLoading || hasAddressLookupError) && (
              <p
                className={`mt-2 text-xs ${
                  hasAddressLookupError ? 'text-destructive' : 'text-on-surface-variant'
                }`}
              >
          ...
          <section aria-label="Delivery Speed">
            <div className="flex items-center justify-start gap-2">
              ...
              <h2 className="text-2xl font-medium text-on-surface">Delivery Speed</h2>
            </div>
        ...
        <CheckoutOrderSummary
          ...
          submissionErrorMessage={
            submitErrors?.errors?.length === 0 ? (submitErrors.message ?? null) : null
          }
