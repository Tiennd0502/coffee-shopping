...
export default function OrdersPageContent() {
  ...
  const [pendingShippingStatusOrderId, setPendingShippingStatusOrderId] = useState<string | null>(
    null,
  );
  const [deleteErrorMessage, setDeleteErrorMessage] = useState<string | null>(null);
  const { mutate: deleteOrderMutate, isPending: isDeletePending } = useDeleteOrder();
  const { mutate: updateOrderStatusMutate } = useUpdateOrderStatus();
  const { mutate: updateOrderShippingStatusMutate } = useUpdateOrderShippingStatus();
  ...
  const handleOrderStatusUpdate = (order: Order, nextStatus: ORDER_STATUS) => {
    ...
    updateOrderStatusMutate(
      { id: orderId, status: nextStatus },
      {
        ...
        onSettled: () => {
          setPendingStatusOrderId((current) => (current === orderId ? null : current));
        },
      },
    );
  };

  const handleOrderShippingStatusUpdate = (order: Order, nextShippingStatus: SHIPPING_STATUS) => {
    const orderId = order.id;
    ...
    updateOrderShippingStatusMutate(
      { id: orderId, shippingStatus: nextShippingStatus },
      {
        ...
        onSettled: () => {
          setPendingShippingStatusOrderId((current) => (current === orderId ? null : current));
        },
      },
    );
  };
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <AlertDialog
        ...
        description={
          <p>
            Are you sure you want to delete <b>{pendingOrderLabel}</b>?. This action cannot be
            undone.
          </p>
        }
        textAction="Delete"
        ...
        {isLoading ? (
          ...
          <Table
            ...
            resolveRowClassName={(order) =>
              order.deletedAt ? 'border-b border-border bg-muted-foreground/20' : ''
            }
            renderRow={(order) => (
              <OrderTableRow
                order={order}
                isDeleteDisabled={isDeletePending || Boolean(order.deletedAt)}
                isStatusDisabled={pendingStatusOrderId === order.id || Boolean(order.deletedAt)}
                isShippingStatusDisabled={pendingShippingStatusOrderId === order.id}
