...
jest.mock('@clerk/elements/common', () => {
  return {
    FieldState: ({
      ...
      children: (field: { state: 'idle' | 'error'; message?: string }) => ReactNode;
...
describe('ClerkField', () => {
  ...
  it('renders with label and placeholder', () => {
    ...
    expect(screen.getByTestId('clerk-input').getAttribute('class')).toContain('-webkit-autofill');
  });
  ...
  it('disables the password input and visibility toggle when disabled', () => {
    render(
      <ClerkField name="password" type="password" placeholder="Enter your password" disabled />,
    );
  ...
  it('adds destructive border styling when the field has an error', () => {
    ...
    expect(screen.getByTestId('clerk-input')).toHaveAttribute('aria-invalid', 'true');
    expect(screen.getByTestId('clerk-input')).toHaveClass(
      'border',
      'border-destructive',
      'ring-2',
  ...
  it('shows client error and sets aria-invalid when clientError is provided', () => {
    render(<ClerkField name="email" placeholder="Enter your email" clientError="Invalid email" />);
