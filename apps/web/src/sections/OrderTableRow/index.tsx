...
export interface OrderTableRowProps {
  ...
  onRequestShippingStatusChange?: (order: Order, nextShippingStatus: SHIPPING_STATUS) => void;
...
export function OrderTableRow({
  ...
  const isStatusActionDisabled = isOrderDeleted || isDeleteDisabled || isStatusDisabled;
  ...
  const getOrderStatusLabel = (status: ORDER_STATUS) =>
    ORDER_STATUS_FILTER_OPTIONS.find(
      (option) => option.value !== ALL_ORDER_STATUSES_VALUE && option.value === status,
    )?.label ?? statusP.label;
  ...
  return (
    <>
      <td className="min-w-0 px-6 py-4 align-middle">
        <p title={displayId} className="truncate font-mono text-sm text-muted-foreground">
          {displayId}
        </p>
      </td>
      <td className="min-w-0 px-6 py-4 align-middle">
