...
jest.mock('@/sections/SocialButtons', () => ({
  ...
  default: ({ disabled }: { disabled?: boolean }) => (
    <div data-testid="social-buttons" data-disabled={disabled ? 'true' : 'false'}>
      Social buttons
    </div>
  ),
}));
...
jest.mock('@clerk/elements/sign-up', () => ({
  Root: ({ fallback, children }: { fallback: React.ReactNode; children: React.ReactNode }) => (
    ...
    if (resend) {
      return (
        <button type="button" className={className} disabled={disabled}>
          {children}
          {typeof fallback === 'function' ? fallback({ resendableAfter: 9 }) : null}
        </button>
      );
    }
...
jest.mock('@clerk/elements/common', () => ({
  ...
  Link: ({
    ...
    <Link href={navigate === 'sign-in' ? '/sign-in' : '/'} className={className}>
      {children}
    </Link>
  ),
}));
...
describe('SignUpForm', () => {
  ...
  it('renders start step controls and auth actions', () => {
    ...
    expect(within(start).getByRole('heading', { name: 'Create your account' })).toBeInTheDocument();
    ...
    expect(within(start).getByPlaceholderText('example@gmail.com')).toBeInTheDocument();
    expect(within(start).getByPlaceholderText('Enter your password')).toBeInTheDocument();
    expect(screen.getByTestId('sign-up-captcha')).toBeInTheDocument();
    expect(within(start).getByRole('checkbox', { name: 'Keep me signed in' })).toBeInTheDocument();
    expect(within(start).getByRole('button', { name: 'Sign up' })).toBeInTheDocument();
    expect(within(start).getByRole('link', { name: 'Sign in' })).toBeInTheDocument();
  });
  ...
  it('renders continue step when Clerk needs an extra step', () => {
    ...
    expect(within(cont).getByRole('heading', { name: 'Almost there' })).toBeInTheDocument();
    expect(within(cont).getByRole('button', { name: 'Back' })).toBeInTheDocument();
    expect(within(cont).getByRole('button', { name: /Continue/i })).toBeInTheDocument();
  });
  ...
  it('disables start submit when global loading is active', () => {
    ...
    const start = getStartForm();
    expect(within(start).getByRole('button', { name: 'Sign up' })).toBeDisabled();
  });
  ...
  it('disables start controls while submit is in progress', () => {
    ...
    expect(within(start).getByPlaceholderText('example@gmail.com')).toBeDisabled();
    expect(within(start).getByPlaceholderText('Enter your password')).toBeDisabled();
    expect(within(start).getByRole('checkbox', { name: 'Keep me signed in' })).toBeDisabled();
    expect(within(start).getByRole('button', { name: 'Sign up' })).toBeDisabled();
    expect(screen.getByTestId('social-buttons')).toHaveAttribute('data-disabled', 'true');
  });
  ...
  it('shows client validation on start when required fields are empty', async () => {
    ...
    expect(screen.getByText(ERROR_MESSAGES.FIRST_NAME_REQUIRED)).toBeInTheDocument();
    expect(screen.getByText(ERROR_MESSAGES.LAST_NAME_REQUIRED)).toBeInTheDocument();
    expect(screen.getByText(ERROR_MESSAGES.EMAIL_ADDRESS_REQUIRED)).toBeInTheDocument();
    expect(screen.getByText(ERROR_MESSAGES.PASSWORD_REQUIRED)).toBeInTheDocument();
  });
  ...
  it('shows client validation for an invalid email on start', async () => {
    ...
    await user.type(within(start).getByPlaceholderText('example@gmail.com'), 'not-an-email');
    await user.type(within(start).getByPlaceholderText('Enter your password'), 'password1');
    await user.click(within(start).getByRole('button', { name: 'Sign up' }));

    expect(screen.getByText(ERROR_MESSAGES.EMAIL_ADDRESS_INVALID)).toBeInTheDocument();
  });
});
