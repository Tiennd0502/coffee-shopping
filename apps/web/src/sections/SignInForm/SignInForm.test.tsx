...
jest.mock('@/sections/ClerkField', () => ({
  ...
  default: ({
    ...
    <label>
      {label}
      <input name={name} placeholder={placeholder} disabled={disabled} onChange={onChange} />
...
describe('SignInForm', () => {
  ...
  it('shows client validation messages when email and password are empty', async () => {
    ...
    expect(screen.getByText(ERROR_MESSAGES.IDENTIFIER_REQUIRED)).toBeInTheDocument();
  ...
  it('shows a client validation message for an invalid email', async () => {
    ...
    await user.type(screen.getByPlaceholderText('example@gmail.com'), 'not-an-email');
    await user.type(screen.getByPlaceholderText('Enter your password'), 'secret');
    await user.click(screen.getByRole('button', { name: 'Sign In' }));

    expect(screen.getByText(ERROR_MESSAGES.IDENTIFIER_EMAIL_INVALID)).toBeInTheDocument();
  });
});
