...
export function OrderDetailModal({ order, open, onOpenChange }: OrderDetailModalProps) {
  ...
  const { firstName = '', lastName = '', phoneNumber = '' } = order.addressSnapshot ?? {};
  ...
  return (
    <AlertDialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        ...
        <div className="grid min-h-0 gap-4 overflow-y-auto pr-1 text-sm">
          <div className="grid gap-2 rounded-xl border p-4 md:grid-cols-2">
            ...
            <p>
              ...
              <span className="font-semibold">{formatOrderListDate(order.createdAt)}</span>
            </p>
            <p>
              <span className="text-muted-foreground">Customer: </span>
              <span className="font-semibold">{formatOrderCustomerName(order)}</span>
            </p>
            <p>
        ...
        <AlertDialogFooter>
          <AlertDialogCancel aria-label="Close order details">Close</AlertDialogCancel>
        </AlertDialogFooter>
      </AlertDialogContent>
    </AlertDialog>
  );
