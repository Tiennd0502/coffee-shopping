...
const EditProductForm = ({ product, productId }: EditProductFormProps) => {
  ...
  const [existingGalleryItems, setExistingGalleryItems] = useState<UploadImageGalleryItem[]>([]);
  ...
  const hasStorefrontChanged = isListedOnStorefront !== initialListedOnStorefront;
  const hasTastingNotesChanged = tastingNotes.join('|') !== initialNotes.join('|');
  const hasPrimaryImageChanged =
    Boolean(avatarImage) || (serverPrimaryUrl ?? '') !== (initialPrimaryUrl ?? '');
  ...
  const validateImageInputs = (): boolean => {
    const nextErrors: ImageValidationErrors = {
      avatar: avatarImage || serverPrimaryUrl?.trim() ? '' : ERROR_MESSAGES.PRODUCT_AVATAR_REQUIRED,
      gallery:
  ...
  const onSubmit = async (data: EditProductFormValues) => {
    ...
    const { addImages, removeImageIds, updateImages } = buildProductUpdateImageDiff(
      product.images,
      finalImages,
    );
      ...
      >
        ...
        <article className="rounded-4xl border border-outline-variant/30 bg-surface-container-low p-6">
          ...
          <div className="mt-5 grid gap-4 lg:grid-cols-[1.2fr_1fr_1fr]">
            <div className="space-y-3 p-2">
              ...
              <Slider
                ...
                onValueChange={(values) => {
                  const nextIndex = Array.isArray(values) ? (values[0] ?? 0) : values;
                  setValue('roastLevel', roastLevels[nextIndex] ?? ROAST_LEVEL.LIGHT, {
                    shouldDirty: true,
                  });
                }}
              />
            ...
            <div className="space-y-2">
              <Label className="tracking-wider uppercase">Sourcing Ethics</Label>
              <div className="flex items-center gap-3">
                <button
                  type="button"
                  ...
                  tastingNotes.map((note) => (
                    ...
                    >
                      ...
                      <button
                        ...
                        onClick={() =>
                          setTastingNotes((prev) => prev.filter((item) => item !== note))
                        }
                      >
