...
export default function CartPageContent() {
  ...
  const { items, totals, isLoading, isError, errorMessage, changeQuantity, removeItem, refetch } =
    useCartStore();
  ...
  if (isLoading) {
    return (
      <div className="mx-auto max-w-7xl px-4 py-8 md:px-6 md:py-12" data-testid="cart-loading">
  ...
  if (isError) {
    return (
      <div className="mx-auto max-w-4xl px-4 py-16 text-center md:px-6" data-testid="cart-error">
        <h1 className="text-3xl font-semibold text-on-surface">Your Sensory Cart</h1>
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 md:px-6 md:py-12">
          ...
          <div className="flex items-center gap-3 rounded-2xl bg-sidebar-accent/80 p-3">
            ...
            <div className="min-w-0">
              ...
              <p className="text-sm text-on-surface-variant">
                {formatCurrency(pendingRemoveItem.unitPrice)} • {pendingRemoveItem.quantity} unit
              </p>
            </div>
          </div>
        )}
      ...
      <section className="space-y-2" aria-label="Cart heading">
        <h1 className="text-4xl font-semibold text-on-surface md:text-5xl">Your Sensory Cart</h1>
      ...
      {!hasItems ? (
        <section className="mt-10 rounded-3xl border border-outline-variant/70 bg-surface-container-low p-10 text-center">
          ...
          <Button className="mt-6 w-auto px-8" onClick={() => router.push(ROUTES.HOME)}>
            Continue Shopping
          </Button>
        </section>
      ) : (
