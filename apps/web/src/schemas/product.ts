...
export const createProductFormSchema = z
  .object({
    categoryId: z.string().trim().min(1, { message: ERROR_MESSAGES.FIELD_REQUIRED }),
    name: z.string().trim().min(1, { message: ERROR_MESSAGES.FIELD_REQUIRED }),
    description: z.string().trim().min(1, { message: ERROR_MESSAGES.FIELD_REQUIRED }),
  ...
  .superRefine((data, ctx) => {
    ...
    if (data.discountType === DISCOUNT_TYPE.FIXED && data.discountValue >= data.price) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['discountValue'],
