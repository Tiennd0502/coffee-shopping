...
describe('parseCreateCategoryForm', () => {
  ...
  it('rejects empty name', () => {
    ...
    if (!result.success) {
      expect(result.error.flatten().fieldErrors.name?.[0]).toBe(ERROR_MESSAGES.FIELD_REQUIRED);
    }
  });
  ...
  it('rejects name longer than max', () => {
    ...
    expect(result.success).toBe(false);
    if (!result.success) {
      expect(result.error.flatten().fieldErrors.name?.[0]).toBe(ERROR_MESSAGES.CATEGORY_NAME_MAX);
    }
  });
});
