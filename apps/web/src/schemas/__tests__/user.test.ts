...
import { parseSignInCredentialsForm, parseSignUpStartForm } from '@/schemas/user';
...
describe('parseSignInCredentialsForm', () => {
  it('accepts a valid email and non-empty password', () => {
    const result = parseSignInCredentialsForm(toSignInFormData('user@example.com', 'secret'));
  ...
  it('rejects an invalid email', () => {
    const result = parseSignInCredentialsForm(toSignInFormData('not-an-email', 'secret'));
...
describe('parseSignUpStartForm', () => {
  ...
  it('rejects empty profile or credentials', () => {
    expect(parseSignUpStartForm(fullStartForm('', '', '', '')).success).toBe(false);
  });
  ...
  it('rejects an invalid email', () => {
    ...
    if (!result.success) {
      expect(
        result.error.issues.some((i) => i.message === ERROR_MESSAGES.EMAIL_ADDRESS_INVALID),
      ).toBe(true);
    }
  });
  ...
  it('rejects a password shorter than 8 characters', () => {
    ...
    if (!result.success) {
      expect(
        result.error.issues.some((i) => i.message === ERROR_MESSAGES.PASSWORD_MIN_LENGTH),
      ).toBe(true);
    }
  });
  ...
  it('rejects empty first or last name', () => {
    expect(
      parseSignUpStartForm(fullStartForm('', 'Lovelace', 'user@example.com', 'password1')).success,
    ).toBe(false);
    expect(
      parseSignUpStartForm(fullStartForm('Ada', '', 'user@example.com', 'password1')).success,
    ).toBe(false);
  });
});
