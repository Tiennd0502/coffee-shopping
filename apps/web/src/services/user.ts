...
function readDeletedAt(payload: Record<string, unknown>): string | null {
  const raw = payload.deletedAt !== undefined ? payload.deletedAt : payload.deleted_at;
...
function normalizeUserListItem(item: unknown): User | null {
  ...
  const firstName = readString(payload.firstName) ?? readString(payload.first_name);
...
> {
  ...
  const users = rawList.map(normalizeUserListItem).filter((u): u is User => u !== null);
