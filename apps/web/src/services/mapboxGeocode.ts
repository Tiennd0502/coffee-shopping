...
function extractVietnamDistrictFromText(text: string): string {
  const match = /\b((?:Quận|Quan|Q\.|Huyện|Huyen|H\.|Thị xã|Thi xa|TX\.)\s*[^,]+)/iu.exec(text);
  return match?.[1]?.trim() ?? '';
}
...
export function mapboxFeatureToCheckoutAddress(
  ...
  const districtFromContext = isAdministrativeCode(districtMeta) ? '' : districtMeta;
  const locality = contextText(feature, 'locality');
  const neighborhood = contextText(feature, 'neighborhood');

  const linesBlob = [feature.place_name, locality, neighborhood, districtMeta].join(' ');
  ...
  } else if (!district && locality && /quận|quan|huyện|huyen/iu.test(locality)) {
    district = locality;
...
function mapFeatureToSuggestion(feature: MapboxGeocodingFeature): MapboxAddressSuggestion | null {
...
export async function findMapboxAddressSuggestions(
  query: string,
): Promise<{ ok: true; suggestions: MapboxAddressSuggestion[] } | { ok: false; error: string }> {
  ...
  url.searchParams.set('types', 'address,place,locality,neighborhood,postcode,region,district');
