import { formatUserListEmailForDisplay, formatPrice, getNameInitials } from '../common';
...
describe('formatUserListEmailForDisplay', () => {
  ...
  it('uses index 1 when only delete_*@* (two segments)', () => {
    expect(formatUserListEmailForDisplay('deleted_user@example.com')).toBe('user@example.com');
  });

  it('returns plain email unchanged when there is no delete prefix', () => {
    expect(formatUserListEmailForDisplay('user@example.com')).toBe('user@example.com');
