...
export function getOrderStatusPresentation(status: ORDER_STATUS): {
  ...
  switch (status) {
    case ORDER_STATUS.PENDING:
      return {
        label: 'Pending',
        badgeClassName: 'bg-amber-100 text-amber-900 dark:bg-amber-950/50 dark:text-amber-200',
      };
    case ORDER_STATUS.CONFIRMED:
      return {
        label: 'Confirmed',
        badgeClassName: 'bg-sky-100 text-sky-900 dark:bg-sky-950/50 dark:text-sky-200',
      };
...
export function getShippingStatusPresentation(status: SHIPPING_STATUS): {
  ...
  switch (status) {
    ...
    case SHIPPING_STATUS.RETURNED:
      return {
        label: 'Returned',
        badgeClassName: 'bg-orange-100 text-orange-900 dark:bg-orange-950/40 dark:text-orange-200',
      };
    default:
      return {
        label: String(status),
...
export function buildOrderDashboardStats(
  ...
  const processing = orders.filter((o) => o.status === ORDER_STATUS.PENDING).length;
  const shipped = orders.filter((o) => o.shippingStatus === SHIPPING_STATUS.SHIPPING).length;
...
export const tagItemErrorsWithLineRefs = (
  ...
  errors.map((error) => {
    ...
    if (!Number.isFinite(apiIndex) || apiIndex < 0 || apiIndex >= linesAtSubmit.length) {
      return error;
    }
    ...
    if (row.submitLineId !== undefined) {
      const displayIndex = items.findIndex(
        (item) =>
          ...
          (row.submitProductId === undefined || item.productId === row.submitProductId),
      );
      if (displayIndex === -1) {
        continue;
      }
