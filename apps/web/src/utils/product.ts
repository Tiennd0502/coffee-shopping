...
export function mapProductToEditFormValues(product: Product): EditProductFormValues {
  return {
...
export function mapProductToFormValues(product: Product): ProductFormValues {
  ...
  const hasDiscount =
    Number(variant?.discountValue) > 0 && Number.isFinite(Number(variant?.discountValue));

  const discountType =
    variant?.discountType === DISCOUNT_TYPE.FIXED ? DISCOUNT_TYPE.FIXED : DISCOUNT_TYPE.PERCENT;
...
export function buildProductUpdateImageDiff(
  ...
  const finalByUrl = new Map(finalImages.map((img) => [normalizeImageUrl(img.url), img]));
  const initialByUrl = new Map(initialImages.map((img) => [normalizeImageUrl(img.url), img]));
  ...
  for (const fin of finalImages) {
    ...
    if (persistedId && (init.sortOrder !== fin.sortOrder || init.isPrimary !== fin.isPrimary)) {
      updateImages.push({
        id: persistedId,
