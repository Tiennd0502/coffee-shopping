import { MENU, MENU_DISABLED_HINT, DASHBOARD_DISABLED_HINT, DASHBOARD_MENU } from '@/constants/nav';
...
describe('nav constants', () => {
  ...
  it('only disables brew guides, subscriptions, and contact in header', () => {
    const disabledByLabel = MENU.filter((item) => item.disabled).map((item) => item.label);
