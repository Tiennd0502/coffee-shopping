...
import type { QueryParams, PaginatedResponse, ResponseSuccess } from '@repo/types';
...
export function useCreateCategory() {
  ...
  return useMutation({
    mutationFn: (payload: CategoryPayload) =>
      apiClient.post<ResponseSuccess<Category>>(API_ROUTES.CATEGORIES, payload),
    onSuccess: () => {
...
export const useCategoryById = (id: string) =>
  useQuery({
    ...
    queryFn: () => apiClient.get<ResponseSuccess<Category>>(`${API_ROUTES.CATEGORIES}/${id}`),
...
export function useUpdateCategory() {
  ...
  return useMutation({
    mutationFn: ({ id, body }: { id: string; body: CategoryPayload }) =>
      apiClient.patch<ResponseSuccess<Category>>(`${API_ROUTES.CATEGORIES}/${id}`, body),
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: ['categories'] });
    },
  });
